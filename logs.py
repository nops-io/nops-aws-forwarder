...
from settings import NOPS_API_KEY
from settings import NOPS_NO_SSL
from settings import NOPS_PORT
from settings import NOPS_SKIP_SSL_VALIDATION
from settings import NOPS_URL
...
class NopsHTTPClient(object):
    ...
    def __init__(self, host, port, no_ssl, skip_ssl_validation, api_key, account_number, scrubber, timeout=10):
        ...
        self._HEADERS.update({"x-aws-account-number": account_number})
        protocol = "http" if no_ssl else "https"
        self._url = "{}://{}:{}/svc/event_collector/v1/cloudtrail_event_collector".format(protocol, host, port)
...
def forward_logs(logs, aws_account_number):
    ...
    cli = NopsHTTPClient(
        NOPS_URL, NOPS_PORT, NOPS_NO_SSL, NOPS_SKIP_SSL_VALIDATION, NOPS_API_KEY, aws_account_number, None
    )
